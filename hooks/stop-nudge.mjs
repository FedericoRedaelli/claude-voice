...
const MAX_NUDGES = 2;
const NUDGE_REASON =
  "Before stopping, call the talk_to_user tool with your final message and any options you offered, so the user can respond by voice. When the tool returns kind=end, you may stop.";
...
function main() {
  ...
  // OPENAI_API_KEY must be set; otherwise talk_to_user can't function and a nudge would just
  // surface as a confusing "stop hook" message in an ordinary session. Text mode always runs.
  const mode = process.env.VOICE_MODE === "text" ? "text" : "voice";
  if (mode === "voice" && !process.env.OPENAI_API_KEY) allow();
